...
players <- read_csv(here::here("exercises/4b/data/players.csv")) %>% clean_names() 

players <- read_csv("data/players.csv") %>% clean_names() 

teams <- read_csv("data/teams.csv") %>% clean_names()

teams <- read_csv(here::here("exercises/4b/data/teams.csv")) %>% clean_names()
```
...
##  Use the `plotly` package to make the previous plot interactive (with the `ggplotly` function). Report the name of the team(s) that has(ve) the highest on both variables (already done by adding label = club)
